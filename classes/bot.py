...
class Bot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        for table in DATABASE_INIT:
            ...
            await self.db.execute(
                f"CREATE TABLE IF NOT EXISTS {table['name']} ({column_list[:-2]})"
            )
        self.logger.info(f"Logged in as {self.user.name}")
        self.logger.info(f"Session ID: {self.session}")
        await self.change_presence(status=Status.online, activity=Game(STATUS))
        await self.wait_until_ready()

    async def on_command_error(
        self, ctx: ApplicationContext | Context, error: DiscordException
    ):
    ...
    async def on_application_command(self, ctx: ApplicationContext):
        ...
        self.logger.info(
            f"{ctx.user}({ctx.user.id}) in {guild}: /{ctx.command.name} {args}"
        )

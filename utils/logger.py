...
class StreamFormatter(Formatter):
    ...
    def format(self, record: LogRecord):
        ...
        levelname_color = (
            COLORS[record.levelname] + record.levelname + RESET + " " * space
        )
...
def setup_logging():
    ...
    filehandler = FileHandler(
        f"logs/{today.strftime('%Y-%m-%d-%H-%M-%S')}.log", "a", "utf-8"
    )
    ...
    errorhandler = FileHandler(
        f"errors/error-{today.strftime('%Y-%m-%d-%H-%M-%S')}.log",
        "a",
        "utf-8",
        delay=True,
    )
    ...
    basicConfig(
        ...
        datefmt="%Y-%m-%d %H:%M:%S",
        handlers=[filehandler, errorhandler, handler],
        level=DEBUG,
    )
